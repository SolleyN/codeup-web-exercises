...
$(`#search`).on(`click`, function () {
  ...
  geocode(userLocation, keys.mapbox).then(function (result) {
    ...
    $.get("https://api.openweathermap.org/data/2.5/forecast", {
      ...
      <ul class="list-group ">
          <li class="list-group-item --bs-info-text text-dark">High: ${parseInt(
            data.list[i].main.temp_max
          )} Low: ${parseInt(data.list[i].main.temp_min)}</li>
          <li class="list-group-item --bs-info-text text-dark"> Weather Description: ${
            data.list[i].weather[0].description
          }</li>
          <li class="list-group-item --bs-info-text text-dark"> Humidity Levels: ${parseInt(
            data.list[i].main.humidity
          )}</li>
          <li class="list-group-item --bs-info-text text-dark"> Barometric Pressure: ${
